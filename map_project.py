...
# 4. ENJIN RSO (Hotine Oblique Mercator - Full Formulation)
def latlon_to_borneo_rso(phi, lam):
    # JUPEM Borneo RSO Constants
    a = 6378137.0                   # GRS80 (GDM2000)
    f = 1/298.257222101
    k0 = 0.99984                    # Scale factor
    phi0 = np.radians(4.0)          # Lat Origin
    lam0 = np.radians(115.0)        # Lon Origin
    gamma0 = np.radians(18.745783)  # Alpha/Azimuth (18 44 44.82)
    E0 = 590476.66                  # False Easting
    N0 = 442857.65                  # False Northing

    e2 = 2*f - f**2
    e = np.sqrt(e2)

    # Hotine Oblique Mercator Math
...
def full_transformation(lat_in, lon_in, h_in, dx, dy, dz):
    ...
    # Shift (Simplest 3-param for speed)
    Xg, Yg, Zg = Xw + dx, Yw + dy, Zw + dz
    
    # Cartesian -> GDM2000 Geodetic (GRS80)
    ...
    return np.degrees(phi_g), np.degrees(lon_g), east, north, (Xg, Yg, Zg)
...
dx = st.sidebar.number_input("dX (m)", value=0.0, step=0.01)
dy = st.sidebar.number_input("dY (m)", value=0.0, step=0.01)
dz = st.sidebar.number_input("dZ (m)", value=0.0, step=0.01)

st.title("🛰️ Professional Borneo RSO Module")
st.write("SBEU 3893 Final | WGS84 ➔ GDM2000 ➔ Borneo RSO Grid")
...
with c1:
    ...
    if st.button("🚀 Transform & Map"):
        lat_g, lon_g, east, north, cart = full_transformation(lat_in, lon_in, h_in, dx, dy, dz)
    ...
    if st.session_state.results:
        ...
        st.markdown(f"""
            <div class="result-card">
                <div class="result-label">BORNEO RSO GRID (SABAH)</div>
