...
def set_bg_local(main_bg):
    if os.path.exists(main_bg):
        ...
        st.markdown(
            ...
            /* Sidebar Color: Darker Steel Blue */
            [data-testid="stSidebar"] {{
                background-color: #4682B4 !important;
            }
            /* White text for sidebar readability */
...
with st.expander("📖 View Mathematical Model"):
    st.write("The 7-Parameter Bursa-Wolf model represents a coordinate transformation using 3 translations, 3 rotations, and 1 scale factor:")
    
    st.latex(r'''\mathbf{X}_{L} = \mathbf{T} + (1+S) \mathbf{R} \mathbf{X}_{W}''')
