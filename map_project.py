...
def latlon_to_hom(lat, lon):
    ...
    # Projection Parameters from your request
    lat0 = np.radians(4.0)
    lon0 = np.radians(115.0)
    k0 = 0.99984
    # Azimuth: 53°18'56.9537" -> 53.31582047°
    alpha_c = np.radians(53 + 18/60 + 56.9537/3600)
    # Grid Angle: 53°07'48.3685" -> 53.13010236°
    ...
    # Rotate by Grid Angle to get Easting and Northing
...
with col1:
    ...
    if st.button("🚀 Calculate HOM Grid"):
        with col2:
            st.subheader("📤 Output: Borneo HOM (Timbalai 1948)")
...
with st.expander("📖 View Mathematical Process"):
    st.write("Using the Hotine Oblique Mercator (HOM) with specific rectification angles for East Malaysia.")
    
    st.latex(r'''v = \frac{A}{2B} \ln\left(\frac{1-U}{1+U}\right) \quad u = \frac{A}{B} \text{atan2}(...)''')
