...
def latlon_to_borneo_rso(phi_deg, lam_deg):
    ...
    E0, N0 = 707496.724, 660060.126 # Calibration Offsets
...
def full_transformation(lat, lon, h, dx, dy, dz, rx_s, ry_s, rz_s, s_ppm):
    ...
    east, north = latlon_to_borneo_rso(np.degrees(phi_g), np.degrees(lon_g))
...
rx_s = st.sidebar.number_input("rX (sec)", value=0.0)
ry_s = st.sidebar.number_input("rY (sec)", value=0.0)
rz_s = st.sidebar.number_input("rZ (sec)", value=0.0)
scale = st.sidebar.number_input("Scale (ppm)", value=0.0)

st.title("🛰️ Borneo RSO & Cartesian Suite")
...
# 7. VISUAL VERIFICATION & MATHEMATICAL PRINCIPLES
if st.session_state.results:
    st.divider()
